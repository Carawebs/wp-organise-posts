...
class TermScreen extends Screen {
  ...
  public function setTermPostmetaKey() {

    // $current_term = $this->current_screen;
    // var_dump ($current_term);
    // $this->term_postmeta_key = $this->custom_taxonomy . '-' . $current_term;
  ...
  public function ajax_organise_posts_ordering() {
    ...
    $siblings_query = [
      ...
      'meta_key'                => $this->term_postmeta_key,
      'orderby'                 => 'meta_value',
      'order'                   => 'ASC',
    ...
    foreach( $siblings->posts as $sibling ) :
      ...
      if ( $nextid === $sibling->ID ) {
        ...
        // increment post meta
        // ---------------------------------------------------------------------
        // $this->term_postmeta_key
        $test = update_post_meta( $post->ID, $this->term_postmeta_key, $start );

        error_log( "result of update_post_meta() \$post->ID: " . $post->ID );
        error_log( "result of update_post_meta() \$this->term_postmeta_key: " . $this->term_postmeta_key );
      ...
      // get_postmeta( $sibling->ID, $this->term_postmeta_key, true )
      $sibling_meta_order = get_post_meta( $sibling->ID, $this->term_postmeta_key, true );
      //if ( $sibling->menu_order != $start ) {
      if ( $sibling_meta_order != $start ) {

        //update_post_meta( $sibling->ID, $this->term_postmeta_key, $start );
        $test = update_post_meta( $sibling->ID, $this->term_postmeta_key, $start );

        //error_log( "result of update_post_meta() on SIBLING: " . $test );
      ...
      if ( ! $nextid && $previd == $sibling->ID ) {
        ...
        // @TODO
        update_post_meta( $post->ID, $this->term_postmeta_key, $start );
        // wp_update_post(array(
        //   'ID' 			=> $post->ID,
        //   'menu_order' 	=> $start,
        //   'post_parent' 	=> $parent_id
